...
def upgrade():
    ...
    with op.batch_alter_table("post", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column(
                "visibility", sa.Enum(Visibility), nullable=False, server_default="0"
            )
        )
    with op.batch_alter_table("post", schema=None) as batch_op:
        batch_op.execute(
            "UPDATE post SET visibility = CASE WHEN hidden = true THEN 'HIDDEN' ELSE 'PUBLISHED' END"
        )
...
def downgrade():
    ...
    with op.batch_alter_table("post", schema=None) as batch_op:
        batch_op.execute(
            "UPDATE post SET hidden = CASE WHEN visibility = 'HIDDEN' THEN true ELSE false END"
        )
